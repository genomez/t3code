...
type SupportedAssistantImageMime = "image/jpeg" | "image/png";

function supportedMimeType(value: string | undefined): SupportedAssistantImageMime | null {
...
function extensionForMime(mimeType: SupportedAssistantImageMime): ".jpg" | ".png" {
  return mimeType === "image/jpeg" ? ".jpg" : ".png";
}

function safeImageName(value: string | undefined, mimeType: SupportedAssistantImageMime): string {
  ...
  if (candidate.length > 0 && extensionMatches) return candidate;
...
function dataUrlInput(
  ...
  const mimeType = supportedMimeType(parsed?.mimeType);
  if (!parsed || !mimeType) return null;
  return {
    _tag: "data-url",
    dataUrl,
    name: safeImageName(suggestedName, mimeType),
  };
}
...
function rawBase64Input(
  ...
  if (!mimeType || base64.length > MAX_BASE64_IMAGE_CHARS) return null;
  const parsed = parseBase64DataUrl(`data:${mimeType};base64,${base64}`);
  ...
  const visitOutput = (value: unknown, depth: number, nativeCodexItem = false): void => {
    ...
    if (type === "image") {
      ...
      if (Predicate.isObject(source) && stringProperty(source, "type") === "base64") {
        ...
        if (sourceData) add(rawBase64Input(sourceData, sourceMimeType, suggestedName));
        return;
      }
    }
...
export function detectSupportedImageMime(bytes: Uint8Array): SupportedAssistantImageMime | null {
  if (
    bytes.length >= 8 &&
    bytes[0] === 0x89 &&
    bytes[1] === 0x50 &&
...
function decodedImage(input: Exclude<AssistantImageInput, { _tag: "local-file" }>): {
  readonly bytes: Uint8Array;
  readonly mimeType: SupportedAssistantImageMime;
  readonly name: string;
} | null {
  const parsed =
    input._tag === "data-url"
  ...
  const declaredMime = supportedMimeType(parsed?.mimeType);
  if (!parsed || !declaredMime || parsed.base64.length > MAX_BASE64_IMAGE_CHARS) return null;
  const bytes = Buffer.from(parsed.base64, "base64");
  if (bytes.byteLength === 0 || bytes.byteLength > PROVIDER_SEND_TURN_MAX_IMAGE_BYTES) return null;
  const detectedMime = detectSupportedImageMime(bytes);
  if (detectedMime !== declaredMime) return null;
  return { bytes, mimeType: detectedMime, name: safeImageName(input.name, detectedMime) };
}
  ...
  function* (input: {
    ...
    const persistOne = Effect.fn("persistAssistantImageInputs.persistOne")(function* (
      ...
      if (imageInput._tag === "local-file") {
        ...
        const bytes = yield* fs.readFile(imageInput.path);
        const mimeType = detectSupportedImageMime(bytes);
        image =
          mimeType && bytes.byteLength <= PROVIDER_SEND_TURN_MAX_IMAGE_BYTES
            ? { bytes, mimeType, name: safeImageName(imageInput.name, mimeType) }
            : null;
      } else {
        image = decodedImage(imageInput);
      }
      ...
      (imageInput) =>
        persistOne(imageInput).pipe(
          Effect.catchCause((cause) =>
            Effect.logWarning("Failed to persist an assistant image", { cause }).pipe(
              Effect.as(null),
            ),
          ),
        ),
