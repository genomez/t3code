...
describe("background notification status", () => {
  ...
  it("sanitizes command detail before applying the status length limit", () => {
    const localPrefix = `C:\\Users\\jason\\${"deep-folder\\".repeat(30)}`;
    expect(
      deriveBackgroundNotificationText(
        {
          latestTurn: { state: "running" },
          activities: [
            {
              ...
              payload: {
                itemType: "command_execution",
                command: `"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content '${localPrefix}status.txt'"`,
              },
            },
          ],
        },
    ...
    ).toBe("Running: Get-Content status.txt");
  });

  it("uses the safe command fallback for credential-bearing input", () => {
    expect(
      deriveBackgroundNotificationText(
        {
          latestTurn: { state: "running" },
          activities: [
            {
              ...activity("Ran command", "2026-08-12T10:00:00.000Z", 1, "tool.completed"),
              payload: { itemType: "command_execution", command: "deploy --token secret-value" },
            },
          ],
        },
        NOW,
