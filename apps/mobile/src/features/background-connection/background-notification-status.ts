...
function payloadCommand(activity: BackgroundNotificationActivity): string | null {
  return firstNestedString(activity.payload, new Set(["command", "cmd", "script"]));
}
...
function presentActivitySummary(activity: BackgroundNotificationActivity): string {
  ...
  if (itemType === "command_execution" || /\b(?:ran|run|running) command\b/.test(normalized)) {
    ...
    return truncate(
      `${!started && (completed || /^ran\b/.test(normalized)) ? "Ran" : "Running"}: ${command}`,
    );
  }
