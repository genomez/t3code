...
const mocks = vi.hoisted(() => ({
  ...
  acquireAgentReplies: vi.fn(() => vi.fn()),
  relayStop: vi.fn(),
...
vi.mock("../agent-awareness/agentNotificationReplyDrain", () => ({
  acquireAgentNotificationReplyDrain: mocks.acquireAgentReplies,
}));
...
it("shares one runtime across duplicate native task starts and cleans it up once", async () => {
  ...
  await vi.waitFor(() => {
    ...
    expect(mocks.acquireAgentReplies).toHaveBeenCalledOnce();
  ...
  expect(mocks.acquireAgentReplies.mock.results[0]?.value).toHaveBeenCalledOnce();
...
it("cleans up a task that finishes loading after the feature was disabled", async () => {
  ...
  expect(mocks.acquireAgentReplies).not.toHaveBeenCalled();
